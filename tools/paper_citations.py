"""Check (or repair) the `parking.tex:<a>-<b>` citations inside the Lean sources.
...
def main() -> int:
    ...
    for path in sorted((ROOT / "Parking").rglob("*.lean")):
        ...
        block = header_block(text) if str(path).startswith(str(ROOT / "Parking" / "Frozen")) else None
