#!/usr/bin/env python3
"""Build Parking and verify its warnings against the frozen manifest.
...
# Lake prints this once per invocation when a dependency's recorded git url
# differs from the one in the toolchain's manifest.  It is not a Lean
# diagnostic and says nothing about any declaration, and repairing it would
# mean running `lake update`, which the project forbids.
LAKE_NOTICE_RE = re.compile(r"manifest out of date")
...
def main() -> int:
    ...
    if build.returncode != 0:
        print(
            f"check_warnings: FAIL (lake build Parking exited {build.returncode})",
            file=sys.stderr,
        )
        return 1
    ...
    for raw_line in build.stdout.splitlines():
        ...
        if warning is None:
            continue
        if LAKE_NOTICE_RE.search(warning.group(1)):
            continue
