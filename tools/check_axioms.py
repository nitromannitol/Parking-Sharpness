...
of another node that is still a draft.  `Parking.Frozen.insertion_inequality` was
...
probe_failed = r.returncode != 0 or bool(missing)
if probe_failed:
    print("check_axioms: Lean did not resolve every manifest export", file=sys.stderr)
    if r.stdout.strip():
        print(r.stdout.strip(), file=sys.stderr)
    if r.stderr.strip():
        print(r.stderr.strip(), file=sys.stderr)

if "--emit" in sys.argv and not probe_failed:
...
sys.exit(1 if probe_failed or (dirty and "--emit" not in sys.argv) else 0)
