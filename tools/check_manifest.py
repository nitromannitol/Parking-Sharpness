#!/usr/bin/env python3
"""Frozen-statement manifest checker for the Parking formalization.
...
Verifies, against ledger/manifest.yaml:
  ...
  4. every .lean file under Parking/Frozen/ is owned by exactly one node;
  5. `sorry` occurs in the production tree (Parking/, Parking.lean) only inside the
  ...
  8. when a node has `closure_sha256`, its source-level Parking dependency-closure
...
`Parking/**/*.lean` outside `Parking/Frozen/`.  It scans comment- and string-masked
...
manifest node protect those bytes, while this closure covers the unfrozen Parking
support surface that D-015 targets.

This is conservative but not an elaboration-equivalent dependency oracle.
False positives can arise when a local binder shadows a uniquely named Parking
...
can also make the heuristic declaration span incomplete.  Imported non-Parking
...
# A `definition` node is an external input.  FROZEN while it is assumed; SEALED
# once the library proves it and a theorem in the same file discharges it.
DEF_STATES = {"FROZEN", "SEALED"}
...
class SourceDecl:
    """One explicitly named source declaration outside Parking/Frozen/."""
...
def current_namespace(scopes: list[tuple[str, str]]) -> str:
    ...
    for kind, name in scopes:
        if kind != "namespace":
            continue
        if name.startswith("Parking.") or name == "Parking":
            parts = name.split(".")
        else:
...
def source_declarations() -> list[SourceDecl]:
    """Index explicit Parking declarations outside the frozen tree."""
    declarations: list[SourceDecl] = []
    orrw_root = ROOT / "Parking"
    ...
    for path in sorted(orrw_root.rglob("*.lean")):
        ...
        for line_no, line in enumerate(lines):
            ...
            if decl_match:
                ...
                if raw_name.startswith("Parking.") or raw_name == "Parking":
                    full_name = raw_name
                elif namespace:
                    full_name = f"{namespace}.{raw_name}"
                else:
                    full_name = raw_name
                if not full_name.startswith("Parking."):
                    continue
...
class DeclarationResolver:
    ...
    def closure(self, source: str) -> set[SourceDecl]:
        """Return the transitive explicit Parking declaration closure of source."""
...
def main() -> int:
    ...
    frozen_dir = ROOT / "Parking" / "Frozen"
    ...
    prod = [ROOT / "Parking.lean"] + sorted((ROOT / "Parking").rglob("*.lean"))
    for f in prod:
        if not f.exists():
            continue
    ...
    if args.emit:
        ...
        print(
            ...
            f"from {len(declarations)} indexed Parking declarations")
        return 0
    ...
    print(
        ...
        f"{len(declarations)} Parking declarations indexed)")
    return 0
