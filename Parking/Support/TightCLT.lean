...
cutoff rewards (`parking.tex:3192-3203`, Stage 2 of the covariance-to-Gaussian step).

`Parking.orientedBoxReward` is, for each fixed `n`, a finite linear combination of the
i.i.d. scenery `η`, so any linear combination of finitely many box rewards is itself a
single finitely-supported weighted sum `∑_w d(w)·η(w)`.  This module builds the
characteristic-function machinery for such a sum:

- `Parking.realLaw_sq_integral_pos`: the one-site variance `σ² = ∫x² ∂(realLaw ν)` is
  strictly positive (a vanishing variance forces a point mass, contradicting `nonconst`).
- `Parking.exists_taylor_charFun_realLaw`: the quantitative second-order Taylor expansion
  of the one-site characteristic function at `0`, `charFun (realLaw ν) s = 1 - σ²s²/2 +
  O(εs²)` for `s` in any neighborhood of `0`, via the library's `taylor_charFun_two` applied
  to the standardized coordinate `η(0)/σ` and rescaled back through `charFun_map_mul`.
- `Parking.charFun_map_weighted_sum_eq_prod_two`: the characteristic function of a
  finitely-supported weighted sum of the i.i.d. scenery is the product, over the support,
  of the one-site characteristic function at the rescaled argument — independence via
  `iIndepFun_infinitePi` restricted to the support through the subtype inclusion, exactly
  as `Parking.charFun_walkFddLaw` (`TightWalk.lean`) computes the walk's characteristic
  function as a cosine product, generalized from the walk's `±1` steps to a general i.i.d.
  real coordinate.
- `Parking.exists_log_charFun_realLaw`: the combined Taylor and logarithm bound,
  `log ∘ charFun (realLaw ν) = -σ²s²/2 + O(εs²)` near `0`, via the library's quantitative
  complex-logarithm Taylor bound `Complex.norm_log_one_add_sub_self_le` applied to
  `charFun (realLaw ν) s - 1`, which the second-order expansion keeps inside the bound's
  domain of validity.
- `Parking.tendsto_charFun_weighted_scenery_sum_two`: **the scalar triangular-array central
  limit theorem** this module exists for.  For a triangular array of finite supports and
  weights whose squares sum to a limit `V` and are uniformly negligible, the characteristic
  function of the weighted i.i.d. sum converges to the Gaussian one `exp(-σ²Vt²/2)`: the
  same cosine-product-to-Gaussian architecture as `Parking.tendsto_charFun_walkFddLaw`
  (`TightWalk.lean`), with `Parking.exists_log_charFun_realLaw` in place of the walk's exact
...
theorem exists_taylor_charFun_realLaw (ν : Measure ℤ) (hν : CriticalLaw ν) :
  ...
  have hσ2pos : 0 < σ2 := realLaw_sq_integral_pos ν hν
...
scenery is the product of the one-site characteristic function over the support.** -/
theorem charFun_map_weighted_sum_eq_prod_two (ν : Measure ℤ) (hν : CriticalLaw ν)
    (S : Finset (Site 2)) (c : Site 2 → ℝ) (t : ℝ) :
    charFun ((iidLaw 2 (realLaw ν)).map (fun η : Site 2 → ℝ => ∑ w ∈ S, c w * η w)) t
      = ∏ w ∈ S, charFun (realLaw ν) (t * c w) := by
  classical
  haveI := hν.prob
  haveI : IsProbabilityMeasure (realLaw ν) := realLaw_isProbability ν
  haveI : IsProbabilityMeasure (iidLaw 2 (realLaw ν)) := by unfold iidLaw; infer_instance
  set g : Site 2 → ℝ → ℂ := fun w b => Complex.exp (((t * c w * b : ℝ) : ℂ) * Complex.I) with hg
  have hgmeas : ∀ w : Site 2, Measurable (g w) := fun w =>
    (Complex.continuous_exp.comp
      ((Complex.continuous_ofReal.comp (continuous_const.mul continuous_id)).mul
        continuous_const)).measurable
  have hind : iIndepFun (fun (w : Site 2) (η : Site 2 → ℝ) => g w (η w)) (iidLaw 2 (realLaw ν)) :=
    iIndepFun_infinitePi (Ω := fun _ : Site 2 => ℝ) (P := fun _ : Site 2 => realLaw ν)
      (X := fun w b => g w b) (fun w => hgmeas w)
  have hind' := hind.precomp (g := fun w : (S : Finset (Site 2)) => (w : Site 2))
    Subtype.val_injective
  have hmeas : ∀ w : (S : Finset (Site 2)),
      AEStronglyMeasurable (fun η : Site 2 → ℝ => g (w : Site 2) (η (w : Site 2)))
        (iidLaw 2 (realLaw ν)) :=
    fun w => ((hgmeas (w : Site 2)).comp (measurable_pi_apply (w : Site 2))).aestronglyMeasurable
  have hkey : (∫ η, ∏ w : (S : Finset (Site 2)), g (w : Site 2) (η (w : Site 2))
        ∂(iidLaw 2 (realLaw ν)))
      = ∏ w : (S : Finset (Site 2)), ∫ η, g (w : Site 2) (η (w : Site 2)) ∂(iidLaw 2 (realLaw ν)) :=
  ...
  have hchar : charFun ((iidLaw 2 (realLaw ν)).map (fun η : Site 2 → ℝ => ∑ w ∈ S, c w * η w)) t
    ...
    calc charFun ((iidLaw 2 (realLaw ν)).map (fun η : Site 2 → ℝ => ∑ w ∈ S, c w * η w)) t
        = ∫ x, Complex.exp (↑(⟪x, t⟫_ℝ) * Complex.I)
            ∂((iidLaw 2 (realLaw ν)).map (fun η : Site 2 → ℝ => ∑ w ∈ S, c w * η w)) :=
          charFun_apply t
      _ = ∫ η, Complex.exp (↑(⟪(∑ w ∈ S, c w * η w : ℝ), t⟫_ℝ) * Complex.I)
            ∂(iidLaw 2 (realLaw ν)) :=
          integral_map hmeasSum hcont.aestronglyMeasurable
      _ = ∫ η, ∏ w : (S : Finset (Site 2)), g (w : Site 2) (η (w : Site 2))
          ...
          show Complex.exp (↑(⟪(∑ w ∈ S, c w * η w : ℝ), t⟫_ℝ) * Complex.I)
              = ∏ w : (S : Finset (Site 2)), g (w : Site 2) (η (w : Site 2))
          rw [hinner]
...
theorem exists_log_charFun_realLaw (ν : Measure ℤ) (hν : CriticalLaw ν) :
  ...
  have hεTlee2 : εT ≤ ε / 2 := by
    ...
    rw [hεT]; linarith
  obtain ⟨δT, hδTpos, hδT⟩ := exists_taylor_charFun_realLaw ν hν εT hεTpos
  ...
  have hrho := hδT s hsT
  -- The two real squeeze facts extracted from `δcap` and `δε`.
...
i.i.d. sums converges to the Gaussian one**, provided the weights' squares sum to a limit
`V` and are uniformly negligible. -/
theorem tendsto_charFun_weighted_scenery_sum_two (ν : Measure ℤ) (hν : CriticalLaw ν)
    (S : ℕ → Finset (Site 2)) (d : ℕ → Site 2 → ℝ) (t : ℝ) {V : ℝ}
    (hV : Tendsto (fun n => ∑ w ∈ S n, (d n w) ^ 2) atTop (𝓝 V))
    (M : ℕ → ℝ) (hM : Tendsto M atTop (𝓝 0)) (hMd : ∀ n, ∀ w ∈ S n, |d n w| ≤ M n) :
    Tendsto (fun n : ℕ => charFun ((iidLaw 2 (realLaw ν)).map
        (fun η : Site 2 → ℝ => ∑ w ∈ S n, d n w * η w)) t) atTop
      (𝓝 (Complex.exp (-((∫ x : ℝ, x ^ 2 ∂(realLaw ν) : ℝ) : ℂ) * (V : ℂ) * (t : ℂ) ^ 2 / 2))) := by
  haveI := hν.prob
  haveI : IsProbabilityMeasure (realLaw ν) := realLaw_isProbability ν
  set σ2 : ℝ := ∫ x : ℝ, x ^ 2 ∂(realLaw ν) with hσ2
  have hσ2nonneg : 0 ≤ σ2 := by rw [hσ2]; exact integral_nonneg fun x => sq_nonneg x
  have hprodeq : ∀ n : ℕ, charFun ((iidLaw 2 (realLaw ν)).map
      (fun η : Site 2 → ℝ => ∑ w ∈ S n, d n w * η w)) t
      = ∏ w ∈ S n, charFun (realLaw ν) (t * d n w) :=
    fun n => charFun_map_weighted_sum_eq_prod_two ν hν (S n) (d n) t
  have hVnonneg : 0 ≤ V :=
    ge_of_tendsto' hV (fun n => Finset.sum_nonneg fun w _ => sq_nonneg (d n w))
  have hnegligible : ∀ δ' : ℝ, 0 < δ' → ∀ᶠ n : ℕ in atTop, ∀ w ∈ S n, |t * d n w| < δ' := by
    intro δ' hδ'pos
    ...
    · have hδt : (0 : ℝ) < δ' / |t| := by positivity
      ...
      linarith
  -- the sum of the logs converges to the target Gaussian exponent
  have hLsum : Tendsto (fun n : ℕ => ∑ w ∈ S n, Complex.log (charFun (realLaw ν) (t * d n w)))
    ...
    obtain ⟨δ, hδpos, hδ⟩ := exists_log_charFun_realLaw ν hν ε' hε'pos
    have hMev : ∀ᶠ n : ℕ in atTop, ∀ w ∈ S n, |t * d n w| < δ := hnegligible δ hδpos
    ...
    have hVclose : ∀ᶠ n : ℕ in atTop, |∑ w ∈ S n, (d n w) ^ 2 - V| < C := by
      have h := Metric.tendsto_nhds.mp hV C hCpos
      filter_upwards [h] with n hn
      rwa [Real.dist_eq] at hn
    filter_upwards [hMev, hVclose] with n hn hCn
    have hVn : ∑ w ∈ S n, (d n w) ^ 2 ≤ V + 1 := by
      have := (abs_lt.mp hCn).2
      linarith
    have herrbound : ∀ w ∈ S n,
        ‖Complex.log (charFun (realLaw ν) (t * d n w)) +
            (σ2 : ℂ) * ((t * d n w : ℝ) : ℂ) ^ 2 / 2‖
          ≤ ε' * (t * d n w) ^ 2 :=
      fun w hw => (hδ (t * d n w) (hn w hw)).2
    have hsumerr : ‖∑ w ∈ S n, (Complex.log (charFun (realLaw ν) (t * d n w)) +
          (σ2 : ℂ) * ((t * d n w : ℝ) : ℂ) ^ 2 / 2)‖ ≤ ε' * t ^ 2 * ∑ w ∈ S n, (d n w) ^ 2 := by
      calc ‖∑ w ∈ S n, (Complex.log (charFun (realLaw ν) (t * d n w)) +
              (σ2 : ℂ) * ((t * d n w : ℝ) : ℂ) ^ 2 / 2)‖
          ≤ ∑ w ∈ S n, ‖Complex.log (charFun (realLaw ν) (t * d n w)) +
              (σ2 : ℂ) * ((t * d n w : ℝ) : ℂ) ^ 2 / 2‖ := norm_sum_le _ _
        _ ≤ ∑ w ∈ S n, ε' * (t * d n w) ^ 2 := Finset.sum_le_sum herrbound
        _ = ε' * t ^ 2 * ∑ w ∈ S n, (d n w) ^ 2 := by
            rw [Finset.mul_sum]
            refine Finset.sum_congr rfl fun w _ => ?_
            ring
    have hsumdecomp : (∑ w ∈ S n, Complex.log (charFun (realLaw ν) (t * d n w)))
        + (σ2 : ℂ) * (t : ℂ) ^ 2 / 2 * (∑ w ∈ S n, ((d n w : ℂ)) ^ 2)
        = ∑ w ∈ S n, (Complex.log (charFun (realLaw ν) (t * d n w)) +
            (σ2 : ℂ) * ((t * d n w : ℝ) : ℂ) ^ 2 / 2) := by
      rw [Finset.sum_add_distrib, Finset.mul_sum]
      congr 1
      refine Finset.sum_congr rfl fun w _ => ?_
      push_cast
      ring
    have hsumcast : (∑ w ∈ S n, ((d n w : ℂ)) ^ 2) = ((∑ w ∈ S n, (d n w) ^ 2 : ℝ) : ℂ) := by
      push_cast; ring
    rw [Complex.dist_eq]
    have hfinalbound : ‖(∑ w ∈ S n, Complex.log (charFun (realLaw ν) (t * d n w))) -
        (-(σ2 : ℂ) * (V : ℂ) * (t : ℂ) ^ 2 / 2)‖
        ≤ ε' * t ^ 2 * ∑ w ∈ S n, (d n w) ^ 2 +
          σ2 * t ^ 2 / 2 * |∑ w ∈ S n, (d n w) ^ 2 - V| := by
      have hrewrite : (∑ w ∈ S n, Complex.log (charFun (realLaw ν) (t * d n w))) -
          (-(σ2 : ℂ) * (V : ℂ) * (t : ℂ) ^ 2 / 2)
          = (∑ w ∈ S n, (Complex.log (charFun (realLaw ν) (t * d n w)) +
              (σ2 : ℂ) * ((t * d n w : ℝ) : ℂ) ^ 2 / 2)) -
            (σ2 : ℂ) * (t : ℂ) ^ 2 / 2 * (((∑ w ∈ S n, (d n w) ^ 2 : ℝ) : ℂ) - (V : ℂ)) := by
      ...
      have hcast2 : ((σ2 : ℂ) * (t : ℂ) ^ 2 / 2 *
          (((∑ w ∈ S n, (d n w) ^ 2 : ℝ) : ℂ) - (V : ℂ)))
          = (((σ2 * t ^ 2 / 2 * (∑ w ∈ S n, (d n w) ^ 2 - V) : ℝ)) : ℂ) := by
        push_cast; ring
    ...
    have h1 : ε' * t ^ 2 * ∑ w ∈ S n, (d n w) ^ 2 ≤ ε' * B := by
      have hε'nn : 0 ≤ ε' := hε'pos.le
      have ht2nn : 0 ≤ t ^ 2 := sq_nonneg t
      calc ε' * t ^ 2 * ∑ w ∈ S n, (d n w) ^ 2 ≤ ε' * t ^ 2 * (V + 1) := by
            apply mul_le_mul_of_nonneg_left hVn (by positivity)
        _ ≤ ε' * B := by rw [hB]; nlinarith
    have h2 : σ2 * t ^ 2 / 2 * |∑ w ∈ S n, (d n w) ^ 2 - V| < ε / 2 := by
      rw [← hD]
      calc D * |∑ w ∈ S n, (d n w) ^ 2 - V| ≤ D * C :=
  ...
  -- eventually every factor is nonzero, so the product is the exponential of the sum of logs
  obtain ⟨δ₁, hδ₁pos, hδ₁⟩ := exists_log_charFun_realLaw ν hν 1 one_pos
  have hne_ev : ∀ᶠ n : ℕ in atTop, ∀ w ∈ S n, charFun (realLaw ν) (t * d n w) ≠ 0 := by
    filter_upwards [hnegligible δ₁ hδ₁pos] with n hn w hw
    exact (hδ₁ (t * d n w) (hn w hw)).1
  have hprodexp : (fun n : ℕ => ∏ w ∈ S n, charFun (realLaw ν) (t * d n w)) =ᶠ[atTop]
      (fun n : ℕ => Complex.exp (∑ w ∈ S n, Complex.log (charFun (realLaw ν) (t * d n w)))) := by
  ...
  have hexpconv : Tendsto (fun n : ℕ => Complex.exp
      (∑ w ∈ S n, Complex.log (charFun (realLaw ν) (t * d n w)))) atTop
      (𝓝 (Complex.exp (-(σ2 : ℂ) * (V : ℂ) * (t : ℂ) ^ 2 / 2))) :=
    (Complex.continuous_exp.tendsto _).comp hLsum
  have hprodconv : Tendsto (fun n : ℕ => ∏ w ∈ S n, charFun (realLaw ν) (t * d n w)) atTop
      (𝓝 (Complex.exp (-(σ2 : ℂ) * (V : ℂ) * (t : ℂ) ^ 2 / 2))) :=
