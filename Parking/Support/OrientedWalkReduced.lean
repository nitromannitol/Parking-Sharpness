...
Steps 1 to 4 of the proof (`parking.tex:3238-3342`) are discharged here; what is
...
/-- `thm:oriented-walk` from the scaling limit of the divisible mean alone.
Steps 1 to 4 are proved; the inputs are the constant `μ` of
`prop:oriented-scaling` and the activity asymptotic. -/
theorem oriented_walk_of_scaling (hBern : Parking.External.Bernstein)
  ...
  haveI := hν.prob
  obtain ⟨Ca, hCa, hmom2⟩ := exists_oriented_hmom2 hBern hConc ν hν
  obtain ⟨Cc, hCc, herr⟩ := exists_oriented_herr hBern hConc ν hν
  refine ⟨?_, ?_, ?_⟩
  · intro hd2
    subst hd2
    obtain ⟨c₂, hc₂, hlo₂⟩ := exists_meanU_oriented_two_lower ν hν
    obtain ⟨C₂, hC₂, hup₂⟩ := meanU_oriented_two_upper_of_moment ν hν Ca hCa.le hmom2
  ...
  · intro hd3
    obtain ⟨Cb, hCb, hmom⟩ := exists_oriented_hmom hBern hConc d hd3 ν hν
    obtain ⟨c₃, C₃, hc₃, hc₃C₃, hlog⟩ :=
      exists_meanU_oriented_log_bounds d hd3 ν hν Cb hCb.le hmom
