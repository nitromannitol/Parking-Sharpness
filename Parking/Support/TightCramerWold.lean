...
scalar characteristic function of every fixed linear combination of its coordinates
...
parking model: it reduces `TendstoInDistribution` of a `Fin m → ℝ`-valued family to the scalar
characteristic-function convergence hypothesis that `Parking.Support.TightFdd`/`TightContFdd`
already supply, via Mathlib's Levy continuity theorem
(`MeasureTheory.ProbabilityMeasure.tendsto_of_tendsto_charFun`) on the auxiliary Euclidean
space `EuclideanSpace ℝ (Fin m)`, transported to the plain product type `Fin m → ℝ` by the
continuous mapping theorem (`MeasureTheory.TendstoInDistribution.continuous_comp`) along the
continuous linear equivalence `EuclideanSpace.equiv`.
-/
import Mathlib
